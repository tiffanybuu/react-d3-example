...
export default function Map(props) {
  ...
  useEffect(() => {
    ...
    if (select === "state") {
      ...
      const time_slider = d3
        .select(".time-slider-svg")
        // .append('svg')
        .attr("width", 1000)
        .attr("height", 70)
        .append("g")
      ...
      function update_spikes(date) {
        const covid_data = covid_cases_states.find(
          (d) =>
        ...
        const length = d3
          .scaleLinear()
          .domain(d3.extent(covid_data.states, (d) => d.covid_rate))
          .range([0, 150]);
        ...
        if (min_rate == max_rate) {
          ...
          let covid_rate_str =
            covid_rate_num < 0.0001 ? "~0%" : formatPercent(covid_rate_num);
          ...
          for (var i = 1; i <= 3; i++) {
            ...
            var covid_rate_num = parseFloat(
              min_rate + (max_rate - min_rate) * (i / 3.0)
            );
            let covid_rate_str =
              covid_rate_num < 0.0001 ? "~0%" : formatPercent(covid_rate_num);
          ...
          if (state != undefined) {
            ...
            d3.select(".tooltip").html(
              "<b>" +
                ...
                (52 -
                  parseFloat(
                    social_capital_states.find(
                      (da) => da.State == lastHovered.properties.name
                    ).rank
                  )) +
                "</b> out of 50 states and DC" +
                "<br/> Covid Case Rate: " +
                rate_str +
                "<br/> Covid Cases: " +
                formatKilo(cases)
      ...
      const g = svg
        ...
        .selectAll("path")
        .data(
          topojson.feature(states_albers, states_albers.objects.states).features
        )
        .join("path")
        ...
        .on("mousemove", (mouseEvent, d) => {
          ...
          var date = slider.value();
          var covid_date = covid_cases_states.find(
            (d) =>
              d.date.getMonth() == date.getMonth() &&
              d.date.getDay() == date.getDay() &&
              d.date.getYear() == date.getYear()
      ...
      // covid cases spike map
      function update_spikes(date) {
        const covid_data = covid_cases_counties.find(
          (d) =>
            d.date.getMonth() == date.getMonth() &&
            d.date.getDay() == date.getDay() &&
            d.date.getYear() == date.getYear()
        ...
        const length = d3
          .scaleLinear()
          .domain(d3.extent(covid_data.counties, (d) => d.covid_rate))
          .range([0, 150]);
          ...
          if (county != undefined) {
            ...
            d3.select(".tooltip").html(
              "<b>" +
                ...
                (2992 -
                  parseFloat(
                    social_capital_counties.find(
                      (da) => da.FIPS_Code == lastHovered.id
                    ).rank
                  )) +
                "</b> out of 2992 counties with data" +
                "<br/> Covid Case Rate: " +
                rate_str +
                "<br/> Covid Cases: " +
                formatKilo(cases)
      ...
      const g = svg
        ...
        .selectAll("path")
        .data(
          topojson.feature(county_albers, county_albers.objects.counties)
            .features
        )
        .join("path")
        ...
        .on("mousemove", (mouseEvent, d) => {
          ...
          var covid_date = covid_cases_counties.find(
            (d) =>
              d.date.getMonth() == date.getMonth() &&
              d.date.getDay() == date.getDay() &&
              d.date.getYear() == date.getYear()
          ...
          d3.select(".tooltip")
            ...
            .html(
              "<b>" +
                ...
                (2992 -
                  parseFloat(
                    social_capital_counties.find((da) => da.FIPS_Code == d.id)
                      .rank
                  )) +
                "</b> out of 2992 counties with data" +
                "<br/> Covid Case Rate: " +
      ...
      <div>
        ...
        <FormControl>
          ...
          >
            ...
            <option value="col">Collective Efficacy</option>
          </Select>
        </FormControl>
      </div>
