...
class Map extends Component {
    constructor(props) {
        super(props)
        // const {f,s,t} = this.props.location.state
        // console.log(this.props.location.state)
        let prop = this.props.location.state;
        // const index_range = d3.extent(this.state.social_capital_states.map(d => d.State_Level_Index))
        // const color = d3.scaleQuantize().domain(index_range).range(["#042698", "#3651ac", "#687cc1", "#9aa8d5", "#ccd3ea"])
        this.state = {
          social_capital_states: prop.social_capital_states,
          states_albers: prop.states_albers,
          covid_cases_states: prop.covid_cases_states,
          index_range: d3.extent(prop.social_capital_states.map(d => d.State_Level_Index)),
          color: d3.scaleQuantize().domain(d3.extent(prop.social_capital_states.map(d => d.State_Level_Index))).range(["#042698", "#3651ac", "#687cc1", "#9aa8d5", "#ccd3ea"])
        };
      }
    componentDidMount() {
        ...
        playButton.on("click", function() {
            if (d3.select(this).text() == "▶️ Play") {
                ...
                //begin animation
                timerID = setInterval(this.animate(slider, timerID, playButton), 50);
            } else {
                this.pauseAnimation(timerID, playButton);
            }
        })
        ...
        .selectAll("path")
        .data(topojson.feature(this.state.states_albers, this.state.states_albers.objects.states).features)
        .join("path")
        .attr("fill",d => this.state.color(d.properties.social_index))
        .attr("d", path)
        ...
        var covid_date = this.state.covid_cases_states.find(d =>
        ...
        +"<br/> Ranks <b>#"+(52 - parseFloat(this.state.social_capital_states.find(da => da.State == d.properties.name).rank)) + "</b> out of 50 states and DC"
        ...
        svg.append("path")
        .attr('transform', "translate(0,70)")
        .datum(topojson.mesh(this.state.states_albers, this.state.states_albers.objects.states, (a, b) => a !== b))
        .attr("fill", "none")
        .attr("stroke", "white")
    ...
    animate(slider, timerID, playButton) {
    ...
    update_spikes(date, svg) {

        const covid_data = (this.state.covid_cases_states.find(d =>
        d.date.getMonth() == date.getMonth() &&
        d.date.getDay() == date.getDay() &&
        d.date.getYear() == date.getYear()
        ))
        ...
        let covid_date = this.state.covid_cases_states.find(d =>
        d.date.getMonth() == date.getMonth() &&
        d.date.getDay() == date.getDay() &&
        d.date.getYear() == date.getYear()
        );
        ...
        +"<br/> Ranks <b>#"+(52 - parseFloat(this.state.social_capital_states.find(da => da.State == lastHovered.properties.name).rank)) + "</b> out of 50 states and DC"
        +"<br/> Covid Case Rate: "+rate_str
        +"<br/> Covid Cases: "+formatKilo(cases))
        }}
    }
    ...
    render() {
        return (
            ...
            <svg class='map' width={svgWidth} height={svgHeight}> 
            </svg>
